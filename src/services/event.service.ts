...
export interface EventPayload {
	eventType: "lesson_completed" | "quiz_completed" | "session_started" | "session_ended";
	contentId?: string;
	courseId?: string;
	timeSpent?: number;
...
export class EventService {
	...
	async processEvent(userId: string, payload: EventPayload): Promise<{ success: boolean; message: string }> {
		try {
			// Verify user exists
			const user = await this.sanityRepository.getUserById(userId);
			if (!user) {
				throw new RepositoryError(`User not found: ${userId}`, new Error("User not found"));
			}
	...
	private async handleSessionStarted(
		...
		const activeSession = await this.sanityRepository.getActiveLearningSession(userId);
		
		if (activeSession) {
			// End the existing session before starting a new one
			const endTime = new Date().toISOString();
			const startTime = new Date(activeSession.startTime || new Date().toISOString());
	...
	private async handleLessonCompleted(
		...
		let activeSession = await this.sanityRepository.getActiveLearningSession(userId);
		if (!activeSession) {
			activeSession = await this.sanityRepository.createLearningSession(userId, payload.courseId);
		}
	...
	private async handleQuizCompleted(
		...
		if (!quizMetadata.percentage) {
			...
			const latestAttempt = await this.sanityRepository.getLatestQuizAttempt(userId, payload.contentId);
			if (latestAttempt) {
				quizMetadata = {
					...quizMetadata,
